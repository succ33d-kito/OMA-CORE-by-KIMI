...
================================
Gestión de SQLite para almacenamiento local de eventos y oportunidades.
Diseñado para operar localmente sin dependencias externas.
"""
...
class OMACoreDatabase:
    """
    Base de datos SQLite para O.M.A.-C.O.R.E.
    Maneja eventos, oportunidades y perfiles de usuario.
    """
        ...
        with self._get_connection() as conn:
            ...
            conn.executescript("""
                ...
                CREATE TABLE IF NOT EXISTS user_profiles (
                    id TEXT PRIMARY KEY,
                    name TEXT,
                    profile_type TEXT NOT NULL, -- trader, creator, founder
                    preferences TEXT, -- JSON
                    watchlist TEXT, -- JSON array de símbolos
    ...
    def insert_event(self, event: Event) -> str:
        ...
        with self._get_connection() as conn:
            conn.execute("""
                ...
                (id, source, source_url, source_id, event_type, category, title, 
    ...
    def get_unprocessed_events(self, limit: int = 100) -> List[Event]:
        """Obtiene eventos no procesados (para el pipeline)"""
        with self._get_connection() as conn:
            rows = conn.execute(
    ...
    def update_event_scores(self, event_id: str, impact: float, relevance: float):
        """Actualiza los scores de impacto y relevancia"""
        with self._get_connection() as conn:
            conn.execute(
                "UPDATE events SET impact_score = ?, relevance_score = ? WHERE id = ?",
                (impact, relevance, event_id)
            )

    def get_event_stats(self) -> Dict[str, Any]:
        """Estadísticas de eventos almacenados"""
        with self._get_connection() as conn:
            ...
            by_urgency = conn.execute(
                "SELECT urgency, COUNT(*) FROM events GROUP BY urgency"
            ).fetchall()
    ...
    def _row_to_event(self, row: sqlite3.Row) -> Event:
        """Convierte una fila SQLite a objeto Event"""
        data = dict(row)
    ...
    def insert_opportunity(self, opportunity: Dict[str, Any]) -> str:
        ...
        with self._get_connection() as conn:
            ...
            """, (
                ...
                opportunity.get("asset_class", ""),
                json.dumps(opportunity.get("assets", [])),
                opportunity.get("score", 0.0),
                opportunity.get("conviction", 0.0),
                opportunity.get("priority", "low"),
                opportunity.get("action_suggested", ""),
    ...
    def get_active_opportunities(self, limit: int = 50) -> List[Dict[str, Any]]:
        """Obtiene oportunidades activas ordenadas por score"""
        with self._get_connection() as conn:
            rows = conn.execute(
                """SELECT o.*, e.title as event_title, e.source, e.event_type
                   ...
                   ORDER BY o.score DESC, o.conviction DESC
                   LIMIT ?""",
                (limit,)
            ).fetchall()
